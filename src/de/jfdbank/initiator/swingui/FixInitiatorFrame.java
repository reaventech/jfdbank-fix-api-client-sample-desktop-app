...
import de.jfdbank.initiator.InitiatorDemoDesktopApplicationMain;
 ...
 */
public class FixInitiatorFrame extends JFrame {

    public FixInitiatorFrame(OrderTableModel orderTableModel, 
            ExecutionTableModel executionTableModel,
            final InitiatorDemoDesktopApplication application) {
        super();
        setTitle("FixInitiatorPanel!");
        setSize(896, 800);

        if (System.getProperties().containsKey("openfix")) {
            createMenuBar(application);
        }
        getContentPane().add(new FixInitiatorPanel(
                orderTableModel, 
                executionTableModel, 
                application),
                BorderLayout.CENTER);
    ...
    private void createMenuBar(final InitiatorDemoDesktopApplication application) {
        ...
        logonItem.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                InitiatorDemoDesktopApplicationMain.get().logon();
            }
        });
        ...
        logoffItem.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                InitiatorDemoDesktopApplicationMain.get().logout();
            }
        });
